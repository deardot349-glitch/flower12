...
export default function ShopClient({ shop }: { shop: Shop }) {
  ...
  const getBadgeLabel = (flower: Flower) => {
    try {
      const freshnessDate = flower.madeAt || flower.createdAt
      const daysAgo = Math.floor((Date.now() - new Date(freshnessDate).getTime()) / (1000 * 60 * 60 * 24))
    ...
    } catch { return { label: 'Нове', color: 'text-gray-700 bg-gray-100' } }
  }
  ...
  return (
    <main className="min-h-screen bg-gray-50" style={{ paddingBottom: 'max(100px, calc(80px + env(safe-area-inset-bottom, 0px)))' }}>
      ...
      <div className="relative">
        <div className="relative overflow-hidden" style={{ height: 'clamp(220px, 45vw, 380px)', background: `linear-gradient(135deg, ${primary}cc, ${accent}cc)` }}>
          {shop.coverImageUrl
            ...
            : <div className="absolute inset-0 flex items-center justify-center opacity-20"><span className="text-8xl">🌸</span></div>
          }
          ...
          <div className="max-w-4xl mx-auto flex items-end gap-4">
            <div className="w-20 h-20 md:w-28 md:h-28 bg-white rounded-2xl shadow-2xl flex items-center justify-center flex-shrink-0 border-[3px] border-white overflow-hidden">
              {shop.logoUrl
                ...
                : <span className="text-3xl md:text-4xl font-black" style={{ color: primary }}>{shop.name.charAt(0).toUpperCase()}</span>
              }
            </div>
      ...
      {shop.allowCustomBouquet && (
        <div className="mx-4 md:mx-auto md:max-w-4xl mt-4 rounded-2xl overflow-hidden shadow-md"
        ...
        {shop.flowers.length === 0 ? (
          ...
          <div className="space-y-3">
            {shop.flowers.map((flower, idx) => {
              const badge = getBadgeLabel(flower)
              ...
              return (
                <div key={flower.id}
                  ...
                  <div className="relative overflow-hidden"
                    style={{ height: imgHeight, background: `linear-gradient(135deg, ${primary}18, ${accent}18)` }}>
                    {flower.imageUrl
      ...
      {/* ===== DELIVERY INFO ===== */}
      {(shop.sameDayDelivery || shop.deliveryZones?.length > 0) && shop.showDeliveryEstimate && (
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-8 py-7">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {shop.showPhone && shop.phoneNumber && (
              <a href={`tel:${shop.phoneNumber}`} className="flex items-center gap-4 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 active:scale-95 transition-all hover:shadow-md">
                <div className="w-12 h-12 bg-green-100 rounded-2xl flex items-center justify-center text-xl flex-shrink-0">📞</div>
                <div><div className="text-xs text-gray-400 font-medium">Подзвонити</div><div className="text-sm font-bold text-gray-900">{shop.phoneNumber}</div></div>
              </a>
            )}
            {shop.showWhatsapp && shop.whatsappNumber && (
              <a href={`https://wa.me/${shop.whatsappNumber.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer"
                ...
                <div><div className="text-xs text-gray-400 font-medium">WhatsApp</div><div className="text-sm font-bold text-gray-900">Написати</div></div>
              </a>
            )}
            {shop.showTelegram && shop.telegramHandle && (
              <a href={`https://t.me/${shop.telegramHandle.replace('@', '')}`} target="_blank" rel="noopener noreferrer"
                ...
                <div><div className="text-xs text-gray-400 font-medium">Telegram</div><div className="text-sm font-bold text-gray-900">{shop.telegramHandle}</div></div>
              </a>
            )}
            {shop.showInstagram && shop.instagramHandle && (
              <a href={`https://instagram.com/${shop.instagramHandle.replace('@', '')}`} target="_blank" rel="noopener noreferrer"
                ...
                <div><div className="text-xs text-gray-400 font-medium">Instagram</div><div className="text-sm font-bold text-gray-900">{shop.instagramHandle}</div></div>
              </a>
            )}
            {shop.showEmail && shop.email && (
              <a href={`mailto:${shop.email}`} className="flex items-center gap-4 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 active:scale-95 transition-all hover:shadow-md">
                <div className="w-12 h-12 bg-gray-100 rounded-2xl flex items-center justify-center text-xl flex-shrink-0">✉️</div>
                <div><div className="text-xs text-gray-400 font-medium">Email</div><div className="text-sm font-bold text-gray-900">{shop.email}</div></div>
              </a>
            )}
            {shop.showLocation && shop.location && (
              <button onClick={openGoogleMaps} className="flex items-center gap-4 bg-white rounded-2xl p-4 shadow-sm border border-gray-100 active:scale-95 transition-all hover:shadow-md text-left w-full">
                <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center text-xl flex-shrink-0">📍</div>
                <div><div className="text-xs text-gray-400 font-medium">Адреса</div><div className="text-sm font-bold text-gray-900">{shop.location}</div></div>
              </button>
            )}
          </div>
      ...
      {shop.flowers.length > 0 && !showModal && (
        <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-white/95 backdrop-blur-md border-t border-gray-200 px-4 pt-3 shadow-2xl">
          <button
            onClick={() => {
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">
                      {selectedFlower.name} · <span className="font-bold" style={{ color: primary }}>{currencySymbol}{selectedFlower.price.toFixed(0)}</span>
                    </p>
                  </div>
                ...
                {submitStatus === 'success' ? (
                  <div className="text-center py-8">
                    ...
                    {estimatedDelivery && deliveryMethod === 'delivery' && (
                      <p className="mb-4 text-sm text-green-700 bg-green-50 rounded-xl p-3">🚚 Очікувана доставка: {estimatedDelivery}</p>
                    )}
                    <a href={`/${shop.slug}/track-order`}
                  ...
                  <>
                            ...
                            <div className="space-y-2">
                              {shop.deliveryZones.map((zone, i) => (
                                <button key={i} onClick={() => setSelectedZone(zone)}
                                  className="w-full flex items-center justify-between px-4 py-3 border-2 rounded-2xl transition-all text-sm active:scale-[0.98]"
                    ...
                    {currentStep === 2 && (
                      <div className="space-y-4">
                        ...
                        {[
                          { label: "Ім'я та прізвище *", key: 'customerName' as const, type: 'text', mode: 'text', auto: 'name', ph: 'Ірина Ковальчук' },
                          { label: 'Номер телефону *', key: 'phone' as const, type: 'tel', mode: 'tel', auto: 'tel', ph: '+380 99 123 45 67' },
                          { label: "Email (необов'язково)", key: 'email' as const, type: 'email', mode: 'email', auto: 'email', ph: 'you@example.com' },
                        ].map(f => (
                          <div key={f.key}>
                            <label className="block text-sm font-semibold text-gray-700 mb-1.5">{f.label}</label>
                            <input type={f.type} inputMode={f.mode as any} autoComplete={f.auto} placeholder={f.ph}
                    ...
                    {currentStep === 4 && (
                      <div className="space-y-4">
                        ...
                        <div className="bg-gray-50 rounded-2xl p-4 space-y-2.5 text-sm border border-gray-100">
                          <div className="flex justify-between items-center"><span className="text-gray-500">Букет</span><span className="font-bold text-gray-900">{selectedFlower.name}</span></div>
                          <div className="flex justify-between items-center"><span className="text-gray-500">Ціна</span><span className="text-lg font-black" style={{ color: primary }}>{currencySymbol}{selectedFlower.price.toFixed(0)}</span></div>
                          <div className="flex justify-between items-center"><span className="text-gray-500">Отримання</span><span className="font-medium">{deliveryMethod === 'pickup' ? '🏪 Самовивіз' : '🚚 Доставка'}</span></div>
                          {selectedZone && (
                            <div className="flex justify-between items-center"><span className="text-gray-500">Зона</span><span className="font-medium">{selectedZone.name} (+{currencySymbol}{selectedZone.fee})</span></div>
                          )}
                          {estimatedDelivery && deliveryMethod === 'delivery' && (
                            <div className="flex justify-between items-center"><span className="text-gray-500">Доставка</span><span className="font-medium text-blue-600">{estimatedDelivery}</span></div>
                          )}
